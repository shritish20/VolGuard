...
st.markdown("""
    ...
    .stApp { background: #121212; color: #FAFAFA; }
    .css-1d391kg { background: #1E1E1E; padding: 20px; border-right: 1px solid #4CAF50; }
    .css-1d391kg h1 { color: #4CAF50; font-size: 1.6em; margin-bottom: 20px; }
    .css-1d391kg .stButton>button { background: #4CAF50; color: #FAFAFA; border-radius: 8px; padding: 10px; font-weight: 500; transition: all 0.3s ease; }
    .css-1d391kg .stButton>button:hover { background: #388E3C; box-shadow: 0 0 15px rgba(76, 175, 80, 0.5); }
    .top-bar { background: #1E1E1E; padding: 15px 20px; border-radius: 8px; margin-bottom: 20px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.5); display: flex; justify-content: space-between; align-items: center; border: 1px solid #4CAF50; }
    .top-bar div { margin: 0 15px; display: flex; align-items: center; }
    .top-bar div p { margin: 0 0 0 8px; font-size: 1.1em; color: #FAFAFA; }
    .top-bar i { color: #4CAF50; }
    .stTabs [role="tab"] { background: transparent; color: #FAFAFA; border-bottom: 2px solid transparent; padding: 10px 20px; margin-right: 5px; transition: all 0.3s ease; }
    .stTabs [role="tab"][aria-selected="true"] { border-bottom: 2px solid #4CAF50; color: #4CAF50; }
    .stTabs [role="tab"]:hover { color: #FFA726; }
    .stButton>button { background: #4CAF50; color: #FAFAFA; border: none; border-radius: 8px; padding: 10px 20px; font-weight: 500; transition: all 0.3s ease; }
    .stButton>button:hover { background: #388E3C; box-shadow: 0 0 15px rgba(76, 175, 80, 0.5); }
    .metric-card { background: #1E1E1E; border-radius: 10px; padding: 15px; margin: 10px 0; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.5); border: 1px solid #4CAF50; transition: transform 0.3s ease; width: 100%; max-width: 600px; }
    .metric-card:hover { transform: scale(1.02); }
    .metric-card h4 { color: #4CAF50; margin: 0; display: flex; align-items: center; }
    .metric-card h4 i { margin-right: 8px; color: #FFA726; }
    .metric-card p { color: #FAFAFA; font-size: 1.1em; margin: 5px 0 0 0; }
    .highlight-card { background: #4CAF50; border-radius: 10px; padding: 15px; margin: 10px 0; box-shadow: 0 4px 10px rgba(76, 175, 80, 0.5); border: 1px solid #388E3C; width: 100%; max-width: 600px; }
    .highlight-card h4 { color: #FAFAFA; margin: 0; display: flex; align-items: center; }
    .highlight-card h4 i { margin-right: 8px; color: #FAFAFA; }
    .highlight-card p { color: #FAFAFA; font-size: 1.3em; margin: 5px 0 0 0; }
    .alert-green { background-color: #388E3C; color: #FAFAFA; padding: 10px; border-radius: 5px; margin: 10px 0; }
    .alert-yellow { background-color: #FFA726; color: #121212; padding: 10px; border-radius: 5px; margin: 10px 0; }
    .alert-red { background-color: #EF5350; color: #FAFAFA; padding: 10px; border-radius: 5px; margin: 10px 0; }
    h1, h2, h3, h4 { color: #4CAF50; }
    .tooltip { position: relative; display: inline-block; }
    .tooltip .tooltiptext { visibility: hidden; width: 200px; background-color: #1E1E1E; color: #FAFAFA; text-align: center; border-radius: 6px; padding: 5px; position: absolute; z-index: 1; bottom: 125%; left: 50%; margin-left: -100px; opacity: 0; transition: opacity 0.3s; }
    .tooltip:hover .tooltiptext { visibility: visible; opacity: 1; }
    @media (max-width: 600px) { .metric-card, .highlight-card { max-width: 100%; } .top-bar { flex-direction: column; align-items: flex-start; } .top-bar div { margin: 5px 0; } }
    </style>
...
max_exposure_pct = st.sidebar.slider("Max Exposure (%)", 10.0, 100.0, st.session_state.risk_settings['max_exposure_pct'], 1.0)
max_loss_per_trade_pct = st.sidebar.slider("Max Loss per Trade (%)", 1.0, 10.0, st.session_state.risk_settings['max_loss_per_trade_pct'], 0.1)
daily_loss_limit_pct = st.sidebar.slider("Daily Loss Limit (%)", 1.0, 10.0, st.session_state.risk_settings['daily_loss_limit_pct'], 0.1)
...
ks', {})
        ...
        if not df.empty:
            df['OI_Skew'] = (df['PE ENSURE_OI'] - df['CE_OI']) / (df['PE_OI'] + df['CE_OI'] + 1)
...
def calculate_regime_score(atm_iv, realized_vol, pcr, vix=15.0, iv_skew_slope=0):
    try:
        ...
        if score > 80:
            ...
            explanation = "Market expects significant volatility. Consider hedged strategies like Iron Fly."
        elif score > 60:
            regime = "Elevated Volatility"
            explanation = "Volatility is above average. Defensive strategies like Iron Condor are suitable."
        elif score > 40:
            regime = "Neutral Volatility"
            explanation = "Market is balanced. Explore strategies like Jade Lizard."
        else:
            regime = "Low Volatility"
            explanation = "Market is calm. Aggressive strategies like Short Straddle may work."
        return score, regime, explanation
    except Exception as e:
        logger.error(f"Regime score error: {e}")
        return 50, "Neutral Volatility", "Unable to classify regime."
...
@retrying.retry(
    ...
    wait_fixed=5000,
    retry_on_exception=lambda e: isinstance(e, ApiException) and e.status == 429
)
def place_order_for_leg(order_api, leg):
    try:
        if not leg["instrument_key"] or not isinstance(leg["instrument_key"], str):
            logger.error(f"Invalid instrument key: {leg['instrument_key']}")
            raise ValueError(f"Invalid instrument key: {leg['instrument_key']}")
        quantity = int(leg["quantity"])
        lot_size = 75
        if quantity % lot_size != 0:
            logger.error(f"Quantity {quantity} is not a multiple of lot size {lot_size}")
            raise ValueError(f"Quantity {quantity} must be a multiple of {lot_size}")
        price_val = 0.0 if leg["order_type"] == "MARKET" else float(leg.get("price", 0.0))
        is_amo = datetime.now().time() < time(9, 15) or datetime.now().time() > time(15, 30)
        ...
        logger.info(f"Order placed: {leg['action']} {leg['instrument_key']} qty={quantity}")
        return response.to_dict()
    except ApiException as e:
        error_msg = f"Order failed for {leg['instrument_key']}: Status={e.status}, Reason={e.reason}, Body={e.body}"
        logger.error(error_msg)
        if e.status == 429:
            logger.warning("Rate limit hit, retrying...")
            raise
        elif e.status == 400 and "invalid instrument" in str(e.body).lower():
            logger.error(f"Instrument {leg['instrument_key']} is invalid or expired")
            raise ValueError(f"Instrument {leg['instrument_key']} is invalid or expired")
        elif e.status == 403 and "insufficient" in str(e.body).lower():
            logger.error("Insufficient funds or margin")
            raise ValueError("Insufficient funds or margin for the order")
        raise Exception(error_msg)
    except Exception as e:
        logger.error(f"Unexpected error placing order for {leg['instrument_key']}: {e}")
        raise
...
def run_volguard(access_token):
    try:
        ...
        if not expiry:
            st.error("Unable to fetch the nearest expiry date.")
        ...
        if not chain:
            ...
            return None, None, None, None, None
        valid_chain = [leg for leg in chain if leg.get('call_options', {}).get('instrument_key') and leg.get('put_options', {}).get('instrument_key')]
        if not valid_chain:
            st.error("No valid instruments found in option chain.")
            logger.error("No valid instruments in option chain")
            return None, None, None, None, None
        spot = chain[0].get("underlying_spot_price") or 0
        if not spot:
            st.error("Unable to fetch spot price.")
            logger.error("Missing spot price")
            return None, None, None, None, None
        df, ce_oi, pe_oi = process_chain(valid_chain)
        if df.empty:
            st.error("Option chain data is empty.")
            logger.error("Processed option chain is empty")
            return None, None, None, None, None
        ...
        st.session_state.option_chain = valid_chain
...
with tab4:
    ...
    strategy_tooltips = {
        ...
        "Short_Straddle": "High reward for minimal movement, but unlimited risk. Use in low volatility.",
    ...
    if run_engine or st.session_state.strategies:
        ...
        selected_strategy = st.selectbox("Select Strategy to Execute", strategy_options)
        quantity = st.number_input("Quantity (Lots)", min_value=1, max_value=100, value=1, step=1)
