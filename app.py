    ...
    if run_button:
        with st.spinner("Running VolGuard Analysis..."):
            ...
            try:
                ...
                st.error(f"Snapshot Tab failed: {str(e)}. Check logs for more details.")
    ...
    with tabs[2]:
        ...
        try:
            ...
            if missing_vars:
                ...
                if strategy is None:
                    st.markdown('<div class="alert-banner">ðŸš¨ Discipline Lock or Risk Limit Breach: Complete Journaling or Reduce Risk to Unlock Trading</div>', unsafe_allow_html=True)
                else:
                    regime_class = {
                        "LOW": "regime-low",
                    ...
                    st.markdown(f"""
                        <div class="strategy-card">
                            ...
                            <p><b>Stop Loss:</b> â‚¹{strategy["Stop_Loss"]:,.0f}</p>
                            <p><b>Take Profit:</b> â‚¹{strategy["Take_Profit"]:,.0f}</p>
                    ...
                    if st.button("Trade Now"):
                        if not st.checkbox("Confirm: I understand trading involves financial risk and market volatility."):
                            st.warning("Please confirm the risk acknowledgment to proceed.")
                        elif st.session_state.trading_halted:
                            st.error("ðŸš¨ Trading Halted: Risk Limits Breached!")
                        else:
                            try:
                                ...
                                st.write(f"Call Sell Strike/Price: {call_sell_strike}/â‚¹{call_sell_price}")
                                st.write(f"Call Buy Strike/Price: {call_buy_strike}/â‚¹{call_buy_price}")
                                st.write(f"Put Sell Strike/Price: {put_sell_strike}/â‚¹{put_sell_price}")
                                st.write(f"Put Buy Strike/Price: {put_buy_strike}/â‚¹{put_buy_price}")
    ...
    with tabs[3]:
        ...
        try:
            if st.session_state.portfolio_data is None:
                ...
                with col1:
                    st.metric("Weekly P&L", f"â‚¹{st.session_state.portfolio_data['weekly_pnl']:,.2f}")
                with col2:
                    st.metric("Margin Used", f"â‚¹{st.session_state.portfolio_data['margin_used']:,.2f}")
                with col3:
                    st.metric("Exposure", f"{st.session_state.portfolio_data['exposure']:.2f}%")
                with col4:
                    st.metric("VaR (5%)", f"â‚¹{st.session_state.portfolio_data['var']:,.2f}")
                with col5:
                    st.metric("CVaR (5%)", f"â‚¹{st.session_state.portfolio_data['cvar']:,.2f}")
                st.markdown("### Stress Test Results")
                if st.session_state.df is not None:
                    stress_results = stress_test_portfolio(st.session_state.df, capital, st.session_state.portfolio_data)
                    for scenario, result in stress_results.items():
                        st.markdown(f"**{scenario}**: P&L â‚¹{result['PnL']:,.2f} ({result['Loss_Pct']:.2f}%)")
                else:
                    st.warning("Data not available for stress test. Run analysis first.")
    ...
    with tabs[4]:
        ...
        try:
            ...
            with st.form("journal_form"):
                ...
                trade_pnl = st.number_input("P&L (â‚¹)", value=0.0, step=100.0)
                ...
                if submitted:
                    ...
                    st.rerun()  # Refresh UI after form submission
    ...
    with tabs[5]:
        ...
        try:
            if st.session_state.backtest_run and st.session_state.backtest_results:
                ...
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("Total P&L", f"â‚¹{results['total_pnl']:,.2f}")
                with col2:
                    st.metric("Win Rate", f"{results['win_rate']*100:.2f}%")
                with col3:
                    st.metric("Max Drawdown", f"â‚¹{results['max_drawdown']:,.2f}")
                with col4:
