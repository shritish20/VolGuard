...
with tab3:
    ...
    xgb_model_url = "https://drive.google.com/uc?export=download&id=1SqEGRJqVgc_UzVpHpPIHWQyddUXuhOjQ"
    xgb_csv_url = "https://raw.githubusercontent.com/shritish20/VolGuard/refs/heads/main/volatility_data.csv"
    ...
    if st.button("Run Model Evaluation"):
        try:
            with st.spinner("Loading XGBoost data and model..."):
                ...
                features = [
                    'ATM_IV', 'Event_Impact_Score', 'IVP', 'Days_to_Expiry',
                    'ATM_IV_minus_RV', 'VIX', '5_Day_Historical_RV', 'IV_Skew',
                    'PCR', 'FII_Derivative_Positioning'
                ]
                target = 'Next_5D_Realized_Vol'  # Assuming this is the target column; adjust if different
                ...
                X = xgb_df[features]
                y = xgb_df[target] * 100  # Convert to percentage if needed
    ...
    try:
        ...
        # Calculate 5-day historical realized volatility
        log_returns = np.log(nifty_df["NIFTY_Close"].pct_change() + 1).dropna()
        five_day_rv = log_returns[-5:].std() * np.sqrt(252) * 100 if len(log_returns) >= 5 else 0
    except Exception as e:
        five_day_rv = 0
        st.warning(f"Could not compute 5-Day Historical Volatility: {e}")

    # Fetch values from session state if available
    ...
    iv_skew = st.session_state.volguard_data.get('iv_skew_data', {}).get('IV_Skew_Slope', 0) if st.session_state.volguard_data else 0
    atm_iv_minus_rv = atm_iv - five_day_rv if atm_iv and five_day_rv else 0
    ...
    with col1:
        ...
        days_to_expiry_input = st.number_input("Days to Expiry", value=5.0, min_value=0.0, max_value=30.0, step=1.0)
        atm_iv_minus_rv_input = st.number_input("ATM IV minus RV (%)", value=float(atm_iv_minus_rv), step=0.1)
    with col2:
        ...
        try:
            with st.spinner("Loading model and predicting..."):
                ...
                new_data = pd.DataFrame({
                    ...
                    'IVP': [ivp_input],
                    'Days_to_Expiry': [days_to_expiry_input],
                    'ATM_IV_minus_RV': [atm_iv_minus_rv_input],
                    'VIX': [vix_input],
                    '5_Day_Historical_RV': [five_day_rv_input],
                    'IV_Skew': [iv_skew_input],
                    'PCR': [pcr_input],
                    'FII_Derivative_Positioning': [fii_derivative_input]
                })
