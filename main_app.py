...
from upstox_api import (initialize_upstox_client, fetch_all_api_portfolio_data,
                        prepare_trade_orders, execute_trade_orders, square_off_positions,
...
st.markdown("""
    <style>
        ...
        /* Metric card styling */
        .stMetric { background: rgba(15, 52, 96, 0.7); border-radius: 15px; padding: 15px; text отличается
...
with st.sidebar:
    ...
    if st.session_state.logged_in:
        ...
        risk_tolerance = st.selectbox("Risk Profile", risk_options, index=risk_options.index(st.session_state.risk_tolerance), key="risk_tolerance_input")
        ...
        start_date = st.date_input("Start Date", value=st.session_state.backtest_start_date, key="backtest_start_date_input")
        st.session_state.backtest_start_date = start_date
        end_date = st.date_input("End Date", value=st.session_state.backtest_end_date, key="backtest_end_date_input")
        st.session_state.backtest_end_date = end_date
        strategy_options = ["All Strategies", "Short Straddle", "Short Strangle", "Iron Condor", "Butterfly Spread",
                           "Iron Fly", "Calendar Spread", "Jade Lizard", "Short Put Vertical Spread",
                           "Short Call Vertical Spread", "Short Put", "Long Put"]
        strategy_choice = st.selectbox("Backtest Strategy Filter", strategy_options,
...
if not is_market_hours():
    st.warning("⚠️ Outside standard market hours (9:15 AM–3:30 PM IST, Mon-Fri). Live trading actions may be limited.")
...
try:
    ...
    if df_loaded is not None and not df_loaded.empty:
        ...
        logger.error(f"Data loading failed: load_data returned None or empty DataFrame from {data_source_tag}.")
    ...
    st.error(f"❌ A critical error occurred during data loading or processing: {str(e)}.")
    data_load_success = False
    st.session_state.analysis_df = None
    st.session_state.real_time_market_data = {}
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs([
    "📊 Market Snapshot", "🔮 Volatility Forecast", "🤖 Trading Strategy", "💼 Portfolio",
    "📝 Journal", "📈 Backtest", "⚠️ Risk Dashboard", "💬 SmartBhai"
])
...
with tab1:
    ...
    if st.session_state.real_time_market_data:
        ...
        if option_chain_df_preview is not None and isinstance(option_chain_df_preview, pd.DataFrame) and not option_chain_df_preview.empty:
            ...
            logger.warning("Option chain data missing or invalid in real_time_market_data.")
    else:
...
with tab2:
    ...
    if st.session_state.analysis_df is not None and not st.session_state.analysis_df.empty:
        try:
            ...
            forecast_log_df, garch_vols, xgb_vols, blended_vols, realized_vol, confidence_score, rmse, feature_importances = forecast_volatility_future(
            ...
            if st.session_state.forecast_log is not None and not st.session_state.forecast_log.empty and 'Blended_Vol' in st.session_state.forecast_log.columns:
                try:
                    ...
                    forecast_chart_data = st.session_state.forecast_log.set_index('Date')['Blended_Vol'].copy()
                    combined_vols = pd.concat([historical_vix, forecast_chart_data]).rename("VIX_Combined")
    ...
    if st.session_state.logged_in and st.session_state.client:
        ...
        if positions_data_updated and isinstance(positions_data_updated, list):
            ...
            if not positions_df.empty:
                display_cols = ["instrument_key", "quantity", "buy_avg_price", "sell_avg_price", "LTP", "CurrentPnL",
                                "unrealized_mtm", "realized_profit", "product"]
            ...
            logger.warning("Position data missing or invalid for display.")
        ...
        if fund_data and isinstance(fund_data, dict):
            ...
            if fund_display_data:
                ...
                for col in fund_df.columns:
                    if pd.api.types.is_numeric_dtype(fund_df[col]):
                        ...
                        fund_df[col] = fund_df[col].apply(lambda x: f"₹{x:.2f}" if pd.notna(x) else 'N/A')
            ...
            logger.warning("Fund data missing or not in dictionary format.")
        ...
        if order_data and isinstance(order_data, list):
            ...
            if not order_df.empty:
                order_display_cols = ["instrument_key", "quantity", "transaction_type", "order_type", "status",
                                     "price", "average_price", "placed_by", "order_id", "exchange_timestamp"]
            ...
            logger.warning("Order book data missing or not in list format.")
        ...
        if trade_data and isinstance(trade_data, list):
            ...
            if not trade_df.empty:
                trade_display_cols = ["instrument_key", "quantity", "transaction_type", "exchange_timestamp",
                                     "price", "order_id", "trade_id"]
            ...
            logger.warning("Trade history data missing or not in list format.")
    else:
...
with tab5:
    ...
    with st.form("journal_form", clear_on_submit=True):
        ...
        date_log = st.date_input("Trade Date", value=date_input_value, key="journal_date_input")
        st.session_state.journal_date_input = date_log
        strategy_input_value = st.session_state.get("journal_strategy_input", "")
        strategy_log = st.text_input("Strategy", value=strategy_input_value, key="journal_strategy_input")
        st.session_state.journal_strategy_input = strategy_log
        pnl_input_value = st.session_state.get("journal_pnl_input", 0.0)
        pnl_log = st.number_input("PnL (₹)", format="%.2f", value=pnl_input_value, key="journal_pnl_input")
        st.session_state.journal_pnl_input = pnl_log
        notes_input_value = st.session_state.get("journal_notes_input", "")
        notes_log = st.text_area("Notes", value=notes_input_value, key="journal_notes_input")
    ...
    if st.session_state.backtest_run and st.session_state.analysis_df is not None and not st.session_state.analysis_df.empty:
        ...
        try:
            ...
            if start_date >= end_date:
                ...
                st.error(f"❌ Backtest Error: Selected date range is outside available data range.")
                logger.error(f"Backtest date range outside available data.")
                st.session_state.backtest_run = False
                st.session_state.backtest_results = None
                st.session_state.backtest_cumulative_pnl_chart_data = None
            else:
                ...
                if backtest_results and isinstance(backtest_results, dict):
                    ...
                    if cumulative_pnl_data is not None and isinstance(cumulative_pnl_data, pd.Series):
                        ...
                        st.warning("Cumulative PnL data not available or in unexpected format.")
                        logger.warning("Cumulative PnL data missing or invalid.")
    ...
    if st.session_state.analysis_df is not None and not st.session_state.analysis_df.empty:
        ...
        try:
            if st.session_state.analysis_df.empty:
                logger.warning("Analysis DataFrame is empty.")
                pnl_day = 0.0
                latest_data = None
            else:
            ...
            with col3:
                ...
                st.markdown(f"<span class='regime-badge {regime_class}'>{regime} Regime</span>", unsafe_allow_html=True)
...
with tab8:
    ...
    st.markdown("<div class='smartbhai-title'>Ask about IV, strategies, or market buzz!</div>", unsafe_allow_html=True)
    st.markdown("<div class='smartbhai-subtitle'>Bhai ko pucho market ka haal!</div>", unsafe_allow_html=True)
    ...
    if smartbhai_gpt:
        ...
        with st.container():
            ...
            with st.form("smartbhai_form", clear_on_submit=True):
                query = st.text_input("Ask SmartBhai:", key="smartbhai_query_input", placeholder="Bhai, IV kya hai? Strategy kya lagau?", label_visibility="collapsed")
                submit_button = st.form_submit_button("Send", use_container_width=True, help="Send message to SmartBhai GPT")
                if submit_button and query:
                    ...
                    try:
                        ...
                        st.session_state.chat_history.append({"role": "assistant", "message": "Sorry, Bhai, kuch technical issue hai. Try again! 🙏"})
                    st.rerun()
    else:
